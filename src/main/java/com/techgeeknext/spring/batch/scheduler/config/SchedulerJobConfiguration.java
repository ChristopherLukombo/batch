...
public class SchedulerJobConfiguration {
	...
	public Step step2() {
		return stepBuilderFactory.get("Step2").listener(employeeStepListener()).listener(employeeReadListener())
				...
				.processor((ItemProcessor<String, String>) item -> String.valueOf(Integer.parseInt(item) * -1)).writer(items -> {
	...
	@Bean
	public Job job3() {
		final Step step = stepBuilderFactory.get("Job3Step").listener(employeeReadListener()).listener(employeeStepListener())
